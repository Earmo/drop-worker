...
export type RelationalDriver = "sqlite" | "mysql" | "postgres";
...
export async function openRelationalMetadataStore(databasePath: string): Promise<OpenRelationalStore> {
  const value = (process.env.DATABASE_DRIVER || "sqlite").trim().toLocaleLowerCase();
  if (value === "sqlite") {
  ...
  if (value === "postgres") return openPostgres();
  if (value === "mysql") return openMySql();
