...
文本和文件条目可以创建公开分享或四位数字口令分享。每个条目最多一个有效链接，有效期可选 1 小时、1 天、7 天或 30 天；进入回收站、到期或手动撤销后立即失效。分享管理页会持续显示有效分享入口并支持复制；受保护链接创建时会把口令放在 URL Fragment 中用于预填，访问者仍须点击确认，之后复制的入口仍需另行输入四位口令。
