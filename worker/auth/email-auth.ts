import type { AuthProvider, Identity } from "../../api/platform";
import {
...
function otpMime(from: string, fromName: string, to: string, code: string): string {
  ...
  const subject = AUTH_OTP_SUBJECT;
  const text = authOtpText(code);
...
export class CloudflareEmailAuth implements AuthProvider {
  ...
  constructor(private readonly env: Env) {
    this.email = normalizeEmail(env.OWNER_EMAIL || "");
    this.fromEmail = normalizeEmail(env.SMTP_FROM || env.AUTH_FROM_EMAIL || "");
  ...
  private async requestOtp(request: Request): Promise<Response> {
    ...
    if (latest && now - latest.created_at < AUTH_RESEND_SECONDS * 1000) {
    ...
    await this.env.DB.batch([
      ...
      this.env.DB.prepare(
        `INSERT INTO auth_challenges (id, email, code_hash, attempts, created_at, expires_at)
         VALUES (?, ?, ?, 0, ?, ?)`,
      ).bind(challengeId, this.email, codeHash, now, now + AUTH_CHALLENGE_SECONDS * 1000),
    ]);
  ...
  private async verifyOtp(request: Request): Promise<Response> {
    ...
    if (!row || row.expires_at <= Date.now() || row.attempts >= AUTH_MAX_ATTEMPTS) {
    ...
    return Response.json(
      ...
      {
        headers: {
          "set-cookie": createSessionCookie(token, true),
  ...
  private async logout(request: Request): Promise<Response> {
    ...
    return Response.json(
      ...
      {
        headers: {
          "set-cookie": createSessionCookie("", true, 0),
          "cache-control": "no-store",
        },
      },
    );
