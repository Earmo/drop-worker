...
import type { AppContext, AuthProvider, Identity } from "../../api/platform";
import { CloudflareEmailAuth } from "../auth/email-auth";
...
async function resolveAccessIdentity(
  request: Request,
  env: Env,
  config: ReturnType<typeof loadCloudflareRuntimeConfig>,
  ...
  if (sitesUserId && sitesEmail) {
    if (config.ownerEmail && normalizeEmail(sitesEmail) !== config.ownerEmail) return null;
  ...
  if (accessToken && env.CF_ACCESS_TEAM_DOMAIN && env.CF_ACCESS_AUD) {
    try {
      ...
      if (config.ownerEmail && normalizeEmail(email) !== config.ownerEmail) return null;
  ...
  if (config.authMode === "development" && (hostname === "localhost" || hostname === "127.0.0.1")) {
    return {
...
export function createCloudflareServices(env: Env): AppContext {
  ...
  const emailAuth = config.authMode === "smtp-otp" ? new CloudflareEmailAuth(env) : null;
  const blobs = new R2BlobStore(env.FILES);
  const auth: AuthProvider = emailAuth ?? {
    mode: config.authMode,
    resolveIdentity: (request) => resolveAccessIdentity(request, env, config),
    handle: async () => null,
  };
